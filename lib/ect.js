	...
	var	ECT = function (options) {
		...
		TemplateContext.prototype.load = function (template) {
			...
			if (ect.options.cache && cache[template]) {
				...
				data = read(file);
				container = this.loadFromString(template, data, file);
				return container;
			}
		};

		TemplateContext.prototype.loadFromString = function (template, data, file) {
			var compiled, container;
		...
		TemplateContext.prototype.renderFromString = function (template_str, data) {
			var that = this;

			var container = this.loadFromString('```' + template_str + '```', template_str);
			var fileInfo = { file : container.file, line : 1 };
		...
		this.renderFromString = function (template_str, data, callback) {
			var context;
			if (typeof arguments[arguments.length - 1] === 'function') {
				if (arguments.length === 2) {
					callback = data;
				...
				context = new TemplateContext(data);
				try {
					callback(undefined, context.renderFromString(template_str));
				} catch (e) {
					callback(e);
				}
			} else {
				context = new TemplateContext(data);
				return context.renderFromString(template_str);
			}
		};
